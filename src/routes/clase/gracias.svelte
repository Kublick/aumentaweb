	...
	<div class="grid lg:grid-cols-4 bg-transparent min-h-[720px] items-center">
		...
		<div class="col-span-3 md:mx-24 mx-4 my-12">
			<div class="flex flex-col gap-6 items-center">
				<h1 class="text-xl text-secondary md:text-3xl font-bold tracking-wide ">
				...
				<div class="flex justify-center gap-4 align-middle items-center ">
					<div
						class="rounded-full  bg-secondary px-6 text-2xl font-bold text-white shadow-lg py-4 my-12 hover:opacity-95"
					>
						{#if disabled}
							<button
								type="button"
								class="inline-flex items-center px-4 py-2 font-semibold leading-6 text-sm  text-white bg-secondary transition ease-in-out duration-150 cursor-not-allowed"
								disabled
							>
								<svg
						...
						{:else}
							<button
								class="font-bold text-sm lg:text-2xl disabled:cursor-not-allowed flex gap-4 bg-secondary"
								on:click={() => {
								...
								{disabled}
							>
								<span class="h-8 w-8 bg-whatsapp rounded-full">
									<svg
										xmlns="http://www.w3.org/2000/svg"
										class="w-8 h-8"
										viewBox="0 0 512 512"
				...
				<p class="w-9/12 md:text-lg">
					2. Ir a tu <span class="font-semibold">BANDEJA</span> de
